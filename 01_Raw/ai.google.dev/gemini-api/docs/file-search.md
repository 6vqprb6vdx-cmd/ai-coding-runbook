...
fetched_at: 2026-09-21T05:42:08.219433+00:00
...
檔案儲存空間和查詢時的嵌入生成作業皆不需付費，您只需在首次為檔案建立索引時支付嵌入費用，以及正常的 Gemini 模型輸入 / 輸出權杖費用。這個新計費模式可讓您更輕鬆且經濟實惠地建構及擴充檔案搜尋工具。詳情請參閱[定價](#pricing)部分。
...
將檔案匯入檔案搜尋儲存庫後，系統會自動將檔案分成多個區塊、嵌入、建立索引，然後上傳至檔案搜尋儲存庫。如要進一步控管分塊策略，可以指定 [`chunking_config`](https://ai.google.dev/api/file-search/file-search-stores?hl=zh-tw#request-body_5) 設定，為每個分塊設定詞元數量上限，以及重疊詞元數量上限。
...
2. **上傳檔案並匯入檔案搜尋儲存庫**：同時上傳檔案並將結果匯入檔案搜尋儲存庫。這會建立暫時的 `File` 物件，也就是原始文件的參照。然後將資料分塊、轉換為檔案搜尋嵌入，並編入索引。物件會在 48 小時後刪除，而匯入檔案搜尋儲存空間的資料則會無限期保留，直到您選擇刪除為止。`File`
...
檔案搜尋儲存庫是文件嵌入的容器。透過 File API 上傳的原始檔案會在 48 小時後刪除，但匯入檔案搜尋儲存庫的資料會無限期儲存，直到您手動刪除為止。你可以建立多個檔案搜尋商店，整理文件。您可以使用 `FileSearchStore` API 建立、列出、取得及刪除檔案，藉此管理檔案搜尋商店。檔案搜尋商店名稱的範圍為全域。
...
如果檔案搜尋儲存庫中有多份文件，而您只想搜尋其中一部分，這項功能就非常實用。
...
使用檔案搜尋時，模型的回覆可能會包含引文，指出生成答案時參考了上傳文件的哪些部分。有助於事實查核和驗證。
...
使用檔案搜尋功能搜尋含有頁面的文件 (例如 PDF 檔案) 時，模型的回覆可能會包含找到資訊的頁碼。您可以透過 `file_citation` 註解的 `page_number` 屬性存取這項資訊。
...
模型在生成內容時參照圖片區塊，API 會在註解中傳回 `file_citation` 類型的註解，其中包含 `media_id`。您可以使用這個 ID 下載模型參照的確切圖片區塊。這個 `media_id` 會在多個搜尋呼叫中持續存在，因此您可以使用 ID 穩定擷取或快取同一張圖片。
...
如果您已在檔案中加入自訂中繼資料，可以在模型回覆的註解中存取該資料。這項功能可將來源文件中的其他內容 (例如網址、頁碼或作者) 傳遞至應用程式邏輯，每個 `file_citation` 類型的引文註解都包含這項自訂中繼資料。
...
| 模型 | 檔案搜尋 |
| --- | --- |
| [Gemini 3.8 Flash](https://ai.google.dev/gemini-api/docs/models/gemini-3.8-flash?hl=zh-tw) | ✔️ |
...
- **專案檔案搜尋儲存空間總大小** (依使用者層級而定)：
  ...
  - **第 2 級**：100 GB
  - **第 3 級**：1 TB
- **建議**：將每個檔案搜尋儲存庫的大小限制在 20 GB 以下，確保最佳的擷取延遲時間。
...
- 儲存空間免費。
- 查詢時嵌入功能不會產生費用。
- 系統會將擷取的文件權杖視為一般[內容權杖](https://ai.google.dev/gemini-api/docs/tokens?hl=zh-tw)計費。
