...
fetched_at: 2026-05-25T12:55:40.160641+00:00
...
- Lanzamos `gemini-3.5-flash`, la versión con disponibilidad general (GA) de [Gemini 3.5 Flash](https://ai.google.dev/gemini-api/docs/models/gemini-3.5-flash?hl=es-419), nuestro modelo más inteligente para un rendimiento de vanguardia sostenido en tareas de agente y de programación.
- Se lanzó la **versión preliminar pública de los agentes administrados en la API de Gemini**. Esto permite a los desarrolladores compilar e implementar agentes autónomos con estado que se ejecutan en entornos de zona de pruebas de Linux seguros y aislados alojados en Google. Para obtener más información, consulta la página [Descripción general de los agentes](https://ai.google.dev/gemini-api/docs/agents?hl=es-419) y la [Guía de inicio rápido](https://ai.google.dev/gemini-api/docs/managed-agents-quickstart?hl=es-419).
- Se lanzó el agente administrado de uso general **Antigravity Agent**, [`antigravity-preview-05-2026`](https://ai.google.dev/gemini-api/docs/models/antigravity-preview-05-2026?hl=es-419), en versión preliminar pública.
  El agente de Antigravity puede planificar, razonar, escribir y ejecutar código de forma autónoma, administrar archivos y navegar por la Web dentro de su contenedor de zona de pruebas. Consulta la guía del [Agente antigravedad](https://ai.google.dev/gemini-api/docs/antigravity-agent?hl=es-419) para obtener muestras de código y especificaciones.
...
- **Próximo cambio que generará interrupciones**: Cambiarán el esquema de solicitud y respuesta (`outputs` → `steps`) y la configuración del formato de salida (`response_format`) de la [API de Interactions](https://ai.google.dev/gemini-api/docs/interactions?hl=es-419). El nuevo esquema se convertirá en el predeterminado el **26 de mayo** y el esquema heredado se quitará el **8 de junio**.
...
- Lanzamos `gemini-robotics-er-1.6-preview`, nuestro modelo de robótica actualizado.
...
- Se introdujeron los nuevos niveles de inferencia [Flex](https://ai.google.dev/gemini-api/docs/flex-inference?hl=es-419) y [Priority](https://ai.google.dev/gemini-api/docs/priority-inference?hl=es-419), que ofrecen más opciones para optimizar el costo o la latencia.
...
- Se lanzó [`gemini-3.1-flash-live-preview`](https://ai.google.dev/gemini-api/docs/models/gemini-3.1-flash-live-preview?hl=es-419), el modelo de audio a audio (A2A) más reciente diseñado para diálogos en tiempo real y aplicaciones de IA que priorizan la voz. Lee la documentación de la [API de Live](https://ai.google.dev/gemini-api/docs/live-api?hl=es-419) para comenzar.

## 25 de marzo de 2026

- Se lanzaron los modelos de generación de música [Lyria 3](https://ai.google.dev/gemini-api/docs/music-generation?hl=es-419): [`lyria-3-clip-preview`](https://ai.google.dev/gemini-api/docs/models/lyria-3-clip-preview?hl=es-419) (clips de 30 segundos) y [`lyria-3-pro-preview`](https://ai.google.dev/gemini-api/docs/models/lyria-3-pro-preview?hl=es-419) (canciones completas). Ambos modelos aceptan entradas de texto e imágenes, y generan audio estéreo de alta calidad a 48 kHz. Consulta la guía de [generación de música](https://ai.google.dev/gemini-api/docs/music-generation?hl=es-419) para obtener detalles y muestras de código.
...
- Se lanzó la versión preliminar de Gemini 3 Flash, `gemini-3-flash-preview`, que ofrece un rendimiento rápido de clase de vanguardia que compite con modelos más grandes a una fracción del costo. Con razonamiento visual y espacial mejorado, y capacidades de agente para programación Lee la documentación sobre algunas funciones nuevas, incluidas las siguientes:
...
- Lanzamos el primer modelo de la serie Gemini 3, `gemini-3-pro-preview`, nuestro modelo de vanguardia para el razonamiento y la comprensión multimodal con potentes capacidades de agente y codificación.
  ...
  - [Niveles de razonamiento](https://ai.google.dev/gemini-api/docs/thinking?hl=es-419#thinking-levels)

  Lee la [Guía para desarrolladores de Gemini 3](https://ai.google.dev/gemini-api/docs/gemini-3?hl=es-419) para obtener información sobre la migración, las nuevas funciones y las especificaciones.
...
- En el caso de [Gemini 2.5 Flash Image](https://ai.google.dev/gemini-api/docs/image-generation?hl=es-419), el recuento de tokens de entrada para las imágenes se redujo de 1,290 a 258, lo que disminuyó el costo de la edición de imágenes.
- Anuncio de baja: Se cerrarán los siguientes modelos:
...
- Se lanzaron los modelos [Veo 3.1 y 3.1 Fast](https://ai.google.dev/gemini-api/docs/video?hl=es-419#veo-3.1) en versión preliminar pública, con nuevas funciones, como las siguientes:
  ...
  - Se pueden usar hasta tres imágenes como referencia para generar un video.
...
- Se lanzó `veo-3.0-generate-preview`, la actualización más reciente de Veo que introduce la generación de videos con audio. Para obtener más información sobre Veo 3, visita la página de [Veo](https://ai.google.dev/gemini-api/docs/video?hl=es-419).
...
- Los modelos preliminares `gemini-2.5-pro-preview-05-06` y `gemini-2.5-pro-preview-03-25` ahora se redireccionan a la versión estable más reciente `gemini-2.5-pro`.
...
- Se lanzó `gemini-2.5-pro-preview-05-06`, una nueva versión de nuestro modelo más potente, con mejoras en la programación y las llamadas a funciones. `gemini-2.5-pro-preview-03-25`
...
- Se actualizaron las [bibliotecas de OpenAI para garantizar la compatibilidad](https://ai.google.dev/gemini-api/docs/openai?hl=es-419).
...
- Se lanzó `gemini-2.0-flash-thinking-exp-01-21`, la versión preliminar más reciente del modelo que impulsa el [modelo Gemini 2.0 Flash Pensar](https://ai.google.dev/gemini-api/docs/thinking?hl=es-419).

## December 19, 2024

**Actualizaciones del modelo:**

- Se lanzó el modo Gemini 2.0 Flash Pensar en versión preliminar pública. El Modo de pensamiento es un modelo de procesamiento en el tiempo de prueba que te permite ver el proceso de pensamiento del modelo mientras genera una respuesta y produce respuestas con capacidades de razonamiento más sólidas.

  Obtén más información sobre el modo Gemini 2.0 Flash Pensar en nuestra [página de descripción general](https://ai.google.dev/gemini-api/docs/thinking-mode?hl=es-419).
...
- Se agregó compatibilidad con instrucciones que incluyen datos de texto, imagen y audio, también conocidas como instrucciones *multimodales*. Para obtener más información, consulta [Cómo generar instrucciones con contenido multimedia](https://ai.google.dev/gemini-api/docs/prompting_with_media?hl=es-419).
