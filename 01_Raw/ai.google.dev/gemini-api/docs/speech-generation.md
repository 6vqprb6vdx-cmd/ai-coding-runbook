...
fetched_at: 2026-07-27T04:46:40.216897+00:00
...
Gemini API 可以使用 Gemini 文字转语音 (TTS) 生成功能将文本输入转换为单个说话者或多个说话者的音频。文字转语音 (TTS) 生成是*[可控](#controllable)*的，这意味着您可以使用自然语言来构建互动，并引导音频的*风格*、*口音*、*节奏*和*音调*。
...
无论是单个说话者还是多个说话者 TTS，您都可以使用自然语言提示词来控制风格、语气、口音和语速。
例如，在单人提示中，您可以说：
...
| **Schedar** - *偶数* | **Gacrux** - *成熟* | **Pulcherrima** - *直率* |
...
若要解锁此功能，用户可以把自己想象成导演，为虚拟配音演员设置表演场景。为了撰写提示，我们建议您考虑以下组成部分：定义角色核心身份和原型的**音频配置文件**；确定实体环境和情感“氛围”的**场景说明**；以及提供有关风格、口音和节奏控制的更精确表演指导的**导演注释**。

通过提供细致的指令，例如精确的地区口音、特定的副语言特征（例如气声）或语速，用户可以利用模型的上下文感知能力生成高度动态、自然且富有表现力的音频表演。为获得最佳效果，我们建议**脚本**和导演提示保持一致，*以便“谁在说”*与*“说了什么”*和*“怎么说的”*保持一致。
...
您还可以精确控制特定部分，也就是说，您可以低声细语地朗读一部分，大声朗读另一部分。
...
- **场景** - 设置舞台。描述实体环境和“氛围”。
- **导演笔记** - 效果指南，您可以在其中详细说明哪些指令对虚拟人才来说非常重要，需要特别注意。例如，风格、呼吸、节奏、发音和口音。
...
仅定义对性能至关重要的内容，并注意不要过度指定。过于严格的规则会限制模型的创造力，并可能导致效果不佳。在角色和场景说明与具体表演规则之间取得平衡。
...
设置生成的语音的语气和风格。包括欢快、活力、放松、无聊等，以指导表演。请尽可能详细地描述，并提供必要的信息：*“富有感染力的热情。听众应该感觉自己正在参与一场盛大而精彩的社区活动。”*比说*“充满活力和热情”*效果更好。
...
- 请务必确保整个提示保持连贯一致，脚本和指令应相互配合，才能打造出精彩的表演。
...
- **提示分类器错误拒绝**：模糊的提示可能无法触发语音合成分类器，导致请求被拒绝 (`PROHIBITED_CONTENT`)，或者导致模型大声朗读您的风格说明和导演注释。通过添加清晰的序言来验证提示，该序言指示模型合成语音，并明确标记实际语音转写内容的开始位置。
