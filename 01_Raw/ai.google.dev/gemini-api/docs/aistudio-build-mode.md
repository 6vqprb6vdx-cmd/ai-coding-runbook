...
fetched_at: 2026-06-29T05:30:43.989198+00:00
...
หน้านี้อธิบายวิธีใช้ Google AI Studio เพื่อสร้าง (หรือ "Vibe Coding") และทำให้ใช้งานได้แอปอย่างรวดเร็วเพื่อทดสอบความสามารถล่าสุดของ Gemini เช่น [Nano Banana](https://ai.google.dev/gemini-api/docs/image-generation?hl=th) และ [Live
...
- **เริ่มต้นด้วยพรอมต์**: ในโหมดสร้าง ให้ใช้กล่องรับข้อมูลเพื่อป้อนคำอธิบายของสิ่งที่คุณต้องการสร้าง เลือกชิป AI เพื่อเพิ่มฟีเจอร์ที่เฉพาะเจาะจง เช่น การสร้างรูปภาพหรือข้อมูล Google Maps ลงในพรอมต์ คุณยัง
...
ที่คุณสามารถแสดงตัวอย่างในโปรแกรมจำลองที่ใช้เบราว์เซอร์ ติดตั้งในอุปกรณ์จริง
...
**Antigravity Agent** คือฟังก์ชัน AI หลักใน [Google
Antigravity](https://antigravity.google?hl=th) และตอนนี้คอมโพเนนต์หลักของ
agent harness กำลังขับเคลื่อนประสบการณ์โหมดสร้างใน Google AI Studio โดยจะทำงานได้มากกว่าการสร้างโค้ดอย่างง่ายด้วยการรักษาบริบทของทั้งโปรเจ็กต์ การจัดการไฟล์หลายไฟล์ และการทำความเข้าใจคำสั่งที่ซับซ้อนเพื่อสร้างแอปพลิเคชันแบบฟูลสแต็กที่แข็งแกร่ง
...
Google AI Studio ปลดล็อกศักยภาพของระบบนิเวศเว็บสมัยใหม่ ซึ่งช่วยให้คุณสร้างได้มากกว่าแค่ต้นแบบฝั่งไคลเอ็นต์
...
- **การจัดการข้อมูลลับ**: จัดเก็บคีย์ API และข้อมูลลับอย่างปลอดภัยในเมนู**การตั้งค่า** ซึ่งจะเข้าถึงได้ในโค้ดฝั่งเซิร์ฟเวอร์ เพื่อให้มั่นใจว่าข้อมูลจะปลอดภัยจากการเปิดเผยฝั่งไคลเอ็นต์
...
- **Firebase Firestore และการตรวจสอบสิทธิ์**: จัดสรรและตั้งค่า Firebase โดยอัตโนมัติ
  ...
  เอเจนต์จะจัดการกระบวนการตั้งค่าทั้งหมดและเขียนโค้ดในแอปสำหรับบริการเหล่านี้ด้วย
...
นอกจากนี้ คุณยังสร้างแอป Android ที่มาพร้อมเครื่องโดยใช้ Kotlin และ Jetpack Compose ได้ด้วย
...
- **ดาวน์โหลดและพัฒนาในเครื่อง**: ส่งออกโค้ดที่สร้างเป็น**ไฟล์ ZIP** แล้วนำเข้าไปยังตัวแก้ไขโค้ด
- **พุชไปยัง GitHub**: ผสานรวมโค้ดกับกระบวนการพัฒนาและการ
  ติดตั้งใช้งานที่มีอยู่โดยการพุชไปยัง**ที่เก็บ GitHub**

## ฟีเจอร์หลัก

Google AI Studio มีฟีเจอร์หลายอย่างที่จะช่วยให้กระบวนการสร้างเป็นไปอย่าง
เป็นภาพและใช้งานง่าย ดังนี้

- **สร้างและทำซ้ำแอป Full Stack**: สร้างแอป Full Stack ด้วยพรอมต์เพียงข้อความเดียว และทำซ้ำผ่านแชทหรือ**โหมดคำอธิบายประกอบ** โหมดคำอธิบายประกอบ
...
เมื่อแอปพลิเคชันพร้อมแล้ว คุณจะนำไปใช้งานได้โดยทำดังนี้
...
- **Cloud Run**: ทำให้แอปพลิเคชันใช้งานได้เป็นบริการที่ปรับขนาดได้
  ...
  ดูข้อมูลเพิ่มเติมเกี่ยวกับการทำให้ใช้งานได้ที่
  [การทำให้ใช้งานได้จาก Google AI Studio](https://ai.google.dev/gemini-api/docs/aistudio-deploying?hl=th)
...
เมื่อสร้างแอปใหม่ที่ใช้ Gemini API แล้ว AI Studio จะกำหนดค่าคีย์ Gemini API เป็นข้อมูลลับในสภาพแวดล้อมฝั่งเซิร์ฟเวอร์ของแอปโดยอัตโนมัติ
คุณดูและจัดการคีย์นี้ได้ในแผง**ข้อมูลลับ**

- **การตั้งค่าอัตโนมัติ**: ระบบจะตั้งค่า `GEMINI_API_KEY` ให้คุณโดยที่คุณไม่ต้องกำหนดค่าด้วยตนเองเพื่อเริ่มสร้าง
...
- **Cloud Run**: เมื่อทําให้ใช้งานได้ใน Cloud Run จาก AI Studio ระบบจะรวมคีย์ API ไว้ในสภาพแวดล้อมฝั่งเซิร์ฟเวอร์อย่างปลอดภัย
  แอปที่ติดตั้งใช้งานจะใช้
  คีย์ API ของคุณสำหรับการเรียก Gemini API ของผู้ใช้ทั้งหมด
- **การดาวน์โหลด ZIP**: หากดาวน์โหลดแอปเป็นไฟล์ ZIP เพื่อดำเนินการที่อื่น คุณจะต้องตั้งค่า`GEMINI_API_KEY`ตัวแปรสภาพแวดล้อม
...
AI Studio Build เป็นแพลตฟอร์มที่ออกแบบมาเพื่อช่วยให้คุณเปลี่ยนจากพรอมต์ง่ายๆ ไปเป็นแอปพลิเคชันที่พร้อมใช้งานจริงซึ่งทำงานด้วยระบบ AI โดยใช้ Gemini
อธิบายสิ่งที่คุณต้องการสร้างด้วยพรอมต์ แล้ว Gemini จะสร้างแอปให้คุณ คุณยังสำรวจแกลเลอรีของเราเพื่อดูสิ่งที่คุณทำได้ด้วย Gemini API
...
การเรียก Gemini API ของแอปจะดำเนินการจากโค้ดฝั่งเซิร์ฟเวอร์โดยใช้คีย์นี้ จึงไม่มีการเปิดเผยคีย์ในเบราว์เซอร์ คุณดูคีย์ API ได้ในแผง**ข้อมูลลับ**ในส่วน
การตั้งค่า

### คีย์ API ของฉันจะแสดงเมื่อแชร์แอปไหม

ไม่ได้ เราจะจัดเก็บคีย์ API เป็นข้อมูลลับฝั่งเซิร์ฟเวอร์และจะไม่รวมไว้ในโค้ดฝั่งไคลเอ็นต์
เมื่อคุณแชร์แอป ผู้ใช้รายอื่นจะใช้แอปได้ แต่จะดูคีย์ API ของคุณไม่ได้

เมื่อแชร์แอปกับผู้อื่น การเรียก API จะนับรวมในขีดจำกัดการใช้งาน
...
ในสภาพแวดล้อมการโฮสต์ เนื่องจากการเรียก Gemini API ทำจากโค้ดฝั่งเซิร์ฟเวอร์ คีย์ของคุณจึงปลอดภัย
...
โปรดแสดงความคิดเห็นหากคุณมีไอเดียที่เฉพาะเจาะจง
...
ใช้โซลูชันพื้นที่เก็บข้อมูลใดก็ได้ที่เชื่อมต่อผ่านเครือข่ายได้ ตราบใดที่ไม่มีไฟร์วอลล์ที่ป้องกันการเข้าถึงจากช่วง IP แบบไดนามิก
...
เรากำหนดให้มีการรับทราบเพิ่มเติมก่อนที่แอปจะเข้าถึง [Navigator API](https://developer.mozilla.org/en-US/docs/Web/API/Navigator) เหล่านี้ได้ เพื่อให้มั่นใจว่าผู้ชมจะทราบว่าแอปใช้เว็บแคมหรืออุปกรณ์อื่นๆ ของตน
...
การผสานรวม GitHub ของ AI Studio ช่วยให้คุณสร้างที่เก็บสำหรับ
งานและคอมมิตการเปลี่ยนแปลงล่าสุดได้ ขณะนี้เรายังไม่รองรับการดึง
การเปลี่ยนแปลงจากระยะไกล
...
เรามีระบบที่ตรวจสอบแอปโดยอัตโนมัติเพื่อให้แน่ใจว่าเป็นไปตาม
นโยบายของเรา หากพบว่าแอปละเมิดนโยบายของเรา เราจะนำแอปดังกล่าวออกจาก AI Studio การละเมิดนโยบายอาจรวมถึงแต่ไม่จำกัดเพียงรายการต่อไปนี้
...
หากแอปถูกแจ้งว่าละเมิดนโยบายและคุณเชื่อว่าเป็นการแจ้งที่
ผิดพลาด คุณสามารถยื่นอุทธรณ์ได้ การละเมิดนโยบายของเราซ้ำๆ อาจ
...
- **การตรวจสอบเนื้อหา:** การปฏิบัติตามข้อกำหนดเพิ่มเติมอาจมีผลกับ
  บริการอื่นๆ ที่แอปของคุณใช้ เช่น
...
มีผลกับการใช้งานแอปที่แนะนำในแกลเลอรีแอปใน AI Studio เว้นแต่จะระบุไว้เป็นอย่างอื่น
