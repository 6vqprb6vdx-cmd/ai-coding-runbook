...
fetched_at: 2026-08-24T02:28:42.878784+00:00
...
Gemini API จะย้ายจากคีย์มาตรฐานเป็นคีย์การตรวจสอบสิทธิ์เพื่อให้มั่นใจว่ามีการใช้งานที่ปลอดภัย

- **คีย์การให้สิทธิ์เป็นค่าเริ่มต้น**: คีย์ API ใหม่ทั้งหมดที่สร้างใน Google AI Studio
  จะสร้างเป็นคีย์การให้สิทธิ์โดยอัตโนมัติ
- **ระบบจะปฏิเสธคีย์ที่ไม่มีข้อจำกัด**: Gemini API จะปฏิเสธคำขอ
  จาก **คีย์มาตรฐานที่ไม่มีข้อจำกัด** คีย์ API มาตรฐานที่มีการใช้ข้อจำกัดอย่างชัดเจนจะยังคงใช้งานได้ ข้อจำกัดนี้จะป้องกันการใช้คีย์ที่ไม่ได้รับอนุญาตซึ่งอาจมีการแชร์แบบสาธารณะหรือลิงก์กับบริการอื่นๆ
...
- **โปรเจ็กต์เริ่มต้น**: หากคุณเป็นผู้ใช้ใหม่ Google AI Studio จะสร้างโปรเจ็กต์ที่อยู่ในระบบคลาวด์ของ Google Cloud และคีย์ API เริ่มต้นให้โดยอัตโนมัติหลังจากที่คุณยอมรับ
  ข้อกำหนดในการให้บริการ คุณเปลี่ยนชื่อโปรเจ็กต์นี้ได้โดยไปที่มุมมอง**โปรเจ็กต์** ในแดชบอร์ด
...
หากไม่มี ให้สร้างไฟล์การกำหนดค่า Bash แล้วเปิดไฟล์ โดยทำดังนี้
...
หากไม่มี ให้สร้างไฟล์การกำหนดค่า Zsh แล้วเปิดไฟล์ โดยทำดังนี้
...
ข้อจำกัดเกี่ยวกับแหล่งที่มาจะจำกัดที่อยู่ IP เว็บไซต์ หรือแอปพลิเคชันที่สามารถใช้คีย์ของคุณได้
...
4. ระบุที่อยู่ IP หรือช่วงที่อยู่ IP ที่อนุญาต แล้วคลิก**บันทึก**
...
หากมีการแชร์คีย์กับ Google API อื่นๆ (ไม่แนะนำ) ให้จำกัดคีย์ใน Cloud Console โดยทำดังนี้ **หมายเหตุ: คำขอ Gemini API ที่ใช้คีย์นี้จะล้มเหลวหลังจากใช้ข้อจำกัดเหล่านี้**
...
5. คลิก**บันทึก** สร้างคีย์ที่แยกต่างหากและจำกัดใน AI Studio เพื่อใช้ Gemini API ต่อไป

### คีย์ที่ไม่ได้ใช้งานซึ่งถูกบล็อก

ตั้งแต่วันที่ 7 พฤษภาคม 2026 เป็นต้นไป Gemini API จะบล็อกคีย์ API ที่ไม่มีข้อจำกัดซึ่งไม่ได้ใช้งานเป็นระยะเวลานาน คีย์เหล่านี้จะแสดงแท็ก**ถูกบล็อก** ใน AI Studio คุณต้องสร้างคีย์ใหม่หรือใช้คีย์ที่มีข้อจำกัดที่มีอยู่เพื่อใช้งานต่อ
