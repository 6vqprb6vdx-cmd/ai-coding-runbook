...
이러한 카테고리는 [`HarmCategory`](https://ai.google.dev/api/rest/v1/HarmCategory?hl=ko)에 정의되어 있습니다. 이러한 필터를 사용하면 사용 사례에 적합한 결과를 조정할 수 있습니다. 예를 들어 동영상 게임 대화를 제작하는 경우 게임의 특성상 *위험*하다고 평가되는 콘텐츠를 더 많이 허용해도 괜찮다고 판단할 수 있습니다.
...
자세한 내용은 [`HarmBlockThreshold`](https://ai.google.dev/api/generate-content?hl=ko#harmblockthreshold) API 참조를 확인하세요.

### 안전 관련 의견

[`generateContent`](https://ai.google.dev/api/generate-content?hl=ko#method:-models.generatecontent)은 안전 피드백을 포함하는 [`GenerateContentResponse`](https://ai.google.dev/api/generate-content?hl=ko#generatecontentresponse)을 반환합니다.

프롬프트 의견은 [`promptFeedback`](https://ai.google.dev/api/generate-content?hl=ko#promptfeedback)에 포함됩니다. `promptFeedback.blockReason`가 설정되면 프롬프트의 콘텐츠가 차단된 것입니다.

대답 후보 의견은 [`Candidate.finishReason`](https://ai.google.dev/api/generate-content?hl=ko#candidate) 및 [`Candidate.safetyRatings`](https://ai.google.dev/api/generate-content?hl=ko#candidate)에 포함됩니다. 대답 콘텐츠가 차단되고 `finishReason`가 `SAFETY`인 경우 `safetyRatings`에서 자세한 내용을 확인할 수 있습니다. 차단된 콘텐츠는 반환되지 않습니다.
...
다음 코드 스니펫은 `GenerateContent` 호출에서 안전 설정을 설정하는 방법을 보여줍니다. 이렇게 하면 증오심 표현(`HARM_CATEGORY_HATE_SPEECH`) 카테고리의 기준점이 설정됩니다. 이 카테고리를 `BLOCK_LOW_AND_ABOVE`로 설정하면 증오심 표현일 가능성이 낮거나 높은 콘텐츠가 차단됩니다. 기준 설정에 관해 알아보려면 [요청별 안전 필터링](#safety-filtering-per-request)을 참고하세요.
...
- LLM으로 개발할 때 안전 고려사항을 전반적으로 살펴보려면 [안전 가이드](https://ai.google.dev/gemini-api/docs/safety-guidance?hl=ko)를 검토하세요.
- [Jigsaw팀](https://developers.perspectiveapi.com/s/about-the-api-score)에서 확률과 심각도를 평가하는 방법 자세히 알아보기
- [Perspective API](https://medium.com/jigsaw/reducing-toxicity-in-large-language-models-with-perspective-api-c31c39b7a4d7)와 같은 안전 솔루션에 기여하는 제품에 대해 자세히 알아보세요.
  \* 이러한 안전 설정을 사용하여 유해성 분류기를 만들 수 있습니다. 시작하려면 [분류 예시](https://ai.google.dev/examples/train_text_classifier_embeddings?hl=ko)를 참고하세요.
