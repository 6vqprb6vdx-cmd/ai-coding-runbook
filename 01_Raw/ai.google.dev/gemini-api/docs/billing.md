...
fetched_at: 2026-06-22T06:32:35.128753+00:00
...
Để triển khai các ứng dụng ngay từ chế độ Build (Tạo), bạn có thể sử dụng **Google Cloud Starter Tier** (Bậc khởi đầu của Google Cloud). Bậc này cho phép bạn xuất bản tối đa 2 ứng dụng full stack (đầy đủ chức năng) mà không cần thiết lập dự án trên đám mây hoặc tài khoản thanh toán trên Google Cloud. Hãy xem phần [Triển khai từ Google AI Studio](https://ai.google.dev/gemini-api/docs/aistudio-deploying?hl=vi) để biết thông tin chi tiết và tham khảo [tài liệu về Bậc khởi đầu của Google Cloud](https://docs.cloud.google.com/docs/starter-tier?hl=vi) để biết thêm thông tin.
...
1. Chuyển đến trang [Khoá API](https://aistudio.google.com/api-keys?hl=vi), trang [Dự án](https://aistudio.google.com/projects?hl=vi) của AI Studio hoặc bất kỳ nơi nào bạn thấy nút **Thiết lập thông tin thanh toán** trong AI Studio.
...
5. Tiếp theo, bạn sẽ:
   - Yêu cầu trả trước tối thiểu 100.000 VND để hoàn tất quy trình thiết lập thông tin thanh toán (tức là tài khoản của bạn được tự động chỉ định cho gói thanh toán [Trả trước](#prepay)),
...
Sau khi [liên kết một tài khoản thanh toán](#setup-billing) với dự án, bạn có thể theo dõi trạng thái của tài khoản đó trên [trang Thanh toán của AI Studio](https://aistudio.google.com/billing?hl=vi). Không giống như bậc miễn phí, trạng thái của bậc có tính phí là trạng thái động; mặc dù bậc sử dụng của bạn được xác định dựa trên nhật ký tài khoản, nhưng Gemini API sẽ chỉ xử lý các yêu cầu nếu bạn có số dư tín dụng [Trả trước](#prepay) dương.
...
- "***Thiết lập phương thức thanh toán trả trước***" nếu dự án có tài khoản thanh toán được đính kèm, nhưng cần sử dụng gói thanh toán [Trả trước](#prepay) cần được thiết lập.
...
Bạn có thể trả trước bằng cách [thêm tín dụng](#buy-credits) vào tài khoản hoặc thiết lập tính năng [tự động nạp tiền](#auto-reload). Sau khi bạn mua tín dụng, tín dụng chưa sử dụng sẽ hết hạn sau 12 tháng và [không được hoàn tiền](#refunds), trừ trường hợp sau khi [chuyển sang tài khoản Trả sau](#postpay).
...
Tự động nạp tiền là một tính năng không bắt buộc, giúp tự động nạp tiền vào số dư tín dụng trả trước khi số dư này sắp hết. Điều này giúp tránh bị gián đoạn dịch vụ.

Bạn có thể thiết lập tính năng tự động nạp tiền và xem trạng thái tự động nạp tiền trong thẻ *Số dư tín dụng* trên trang [Thanh toán cho AI Studio](https://aistudio.google.com/billing?hl=vi). Nhấp vào **Thiết lập tính năng tự động nạp tiền** hoặc **Quản lý tính năng tự động nạp tiền** để thiết lập phương thức thanh toán, số tiền nạp và số dư tối thiểu kích hoạt khoản thanh toán nạp tiền.

#### Hạn mức tính phí tự động hằng tháng

Hạn mức tự động tính phí hằng tháng dành cho người dùng trả trước và giúp ngăn chặn các khoản phí không mong muốn do tính năng tự động nạp tiền thường xuyên. Hãy sử dụng tính năng này để đặt hạn mức tối đa cho tính năng tự động nạp tiền trong một chu kỳ thanh toán. Khi tổng số tiền nạp tự động trong một chu kỳ thanh toán đạt đến hạn mức này, hệ thống sẽ tắt tính năng tự động nạp tiền cho đến khi bắt đầu tháng tiếp theo. Các khoản thanh toán một lần mà bạn thực hiện theo cách thủ công sẽ không được tính vào hạn mức này.
...
Trong gói thanh toán trả sau, tài khoản thanh toán trên đám mây của bạn sẽ tích luỹ chi phí và bạn sẽ tự động bị tính phí vào cuối tháng hoặc khi chi phí đạt đến [hạn mức chi tiêu được tự động chỉ định](#tier-spend-caps) dựa trên cấp tài khoản của bạn. Khoản thanh toán sẽ được tính vào phương thức thanh toán được liên kết với tài khoản thanh toán trả sau của bạn. Bạn có thể quản lý tài khoản này trên trang [Thanh toán của AI Studio](https://aistudio.google.com/billing?hl=vi).
...
Thời gian xử lý dữ liệu thanh toán trong AI Studio có thể bị chậm trễ, tối đa khoảng 10 phút. Bạn có thể bị tính phí vượt quá hạn mức dự án nếu dữ liệu thanh toán chưa được xử lý trước khi phát sinh thêm phí.
...
Hạn mức sử dụng hằng tháng được áp dụng cho Gemini API ở cấp [tài khoản thanh toán](#cloud-billing). Mặc dù các giới hạn mặc định được đặt sẵn, nhưng bạn có thể [yêu cầu tăng](https://docs.google.com/forms/d/e/1FAIpQLSdiP6BWJyNNN65lnwnlOr-5Kv0MOFp0jLQyqi_ixVCfddqWBw/viewform?hl=vi) để đáp ứng mức sử dụng cao hơn. Tổng mức chi tiêu được tổng hợp trên tất cả các dự án được liên kết có bật dịch vụ Gemini API. Sau khi tổng số tài khoản tích luỹ đạt đến giới hạn của cấp, dịch vụ sẽ bị tạm dừng cho tất cả các dự án được liên kết với tài khoản thanh toán đó cho đến khi bắt đầu chu kỳ thanh toán tiếp theo (ngày 1 của mỗi tháng).
...
- **Nâng cấp cấp độ**: Sau khi bạn thanh toán thành công hoặc khi bạn đáp ứng [các tiêu chí nâng cấp](#about-billing), hệ thống thường sẽ ghi nhận việc nâng cấp cấp độ trong vòng 10 phút.
...
Tất cả [dự án](https://ai.google.dev/gemini-api/docs/api-key?hl=vi#google-cloud-projects) được liên kết với một tài khoản thanh toán Cloud đều kế thừa cấp sử dụng của tài khoản thanh toán đó cũng như các hạn mức tốc độ và hạn mức tài khoản được liên kết. Nếu bạn [thay đổi một dự án](https://docs.cloud.google.com/billing/docs/how-to/modify-project?hl=vi#change_the_billing_account_for_a_project) từ tài khoản thanh toán này sang tài khoản thanh toán khác, thì cấp của dự án đó, cũng như hạn mức tốc độ và hạn mức tài khoản sau đó, sẽ chuyển sang cấp của tài khoản thanh toán mới.
...
[Khoá API](https://ai.google.dev/gemini-api/docs/api-key?hl=vi) là thông tin đăng nhập được tạo trong một dự án. Khoá API không có chế độ cài đặt thanh toán độc lập; khoá API kế thừa hạn mức theo cấp và trạng thái thanh toán của dự án. Mức sử dụng tích luỹ của tất cả các khoá trong một dự án được tính vào hạn mức chi tiêu của dự án đó và tổng mức chi tiêu của tài khoản thanh toán.
...
Để truy cập vào các cấp cao hơn, bạn phải thiết lập thông tin thanh toán cho dự án của mình. Nhấp vào [**Thiết lập thông tin thanh toán**](#setup-billing) trong Google AI Studio. Thao tác này sẽ hướng dẫn bạn chọn hoặc tạo một tài khoản thanh toán Cloud. Nếu bạn bắt buộc phải sử dụng mô hình thanh toán trả trước, thì quy trình **Thiết lập thông tin thanh toán** sẽ hướng dẫn bạn tạo tài khoản trả trước được liên kết với tài khoản thanh toán Cloud của bạn.

### Tôi có thể sử dụng 1 triệu mã thông báo trong gói miễn phí không?

Cấp miễn phí của Gemini API sẽ khác nhau tuỳ thuộc vào mô hình được chọn. Hiện tại, bạn có thể dùng cửa sổ ngữ cảnh 1 triệu token theo những cách sau:
...
### Khoản tín dụng Google Cloud hoạt động như thế nào với phương thức thanh toán trả trước?

Người dùng trả trước phải [mua Tín dụng trả trước](#buy-credits) trước khi có thể áp dụng bất kỳ khoản tín dụng đủ điều kiện nào của Google Cloud cho việc sử dụng Gemini API. Sau khi bạn có số dư Tín dụng trả trước đang hoạt động, các khoản tín dụng của Google Cloud đủ điều kiện sử dụng Gemini API sẽ được dùng trước số dư Tín dụng trả trước. Khi số dư Tín dụng trả trước trong tài khoản thanh toán đạt mức 0, các khoản tín dụng của Google Cloud sẽ không còn được dùng nữa.
...
Hệ thống [Thanh toán trên Google Cloud](https://cloud.google.com/billing/docs/concepts?hl=vi) sẽ xử lý việc thanh toán cho Gemini API. Tìm hiểu về thông tin thanh toán qua đám mây trong sản phẩm trong [tài liệu về Thanh toán qua đám mây](https://docs.cloud.google.com/billing/docs/in-product-billing-setup?hl=vi).

### Tôi có bị tính phí cho các yêu cầu không thực hiện được không?

Nếu yêu cầu của bạn không thành công và trả về lỗi 400 hoặc 500, thì bạn sẽ không bị tính phí cho các mã thông báo đã sử dụng. Tuy nhiên, yêu cầu đó vẫn được tính vào hạn mức của bạn.
...
Hãy tham khảo [Điều khoản dịch vụ](https://ai.google.dev/gemini-api/terms?hl=vi#paid-services) để biết thông tin chi tiết về cách xử lý dữ liệu khi bạn bật tính năng thanh toán trên Cloud (xem phần "Cách Google sử dụng dữ liệu của bạn" trong "Dịch vụ có tính phí"). Xin lưu ý rằng các câu lệnh của bạn trên Google AI Studio sẽ được xử lý theo các điều khoản tương tự của "Dịch vụ có tính phí" miễn là có ít nhất 1 dự án API đã bật tính năng thanh toán. Bạn có thể xác thực điều này trên [trang khoá Gemini API](https://aistudio.google.com/api-keys?hl=vi) nếu thấy bất kỳ dự án nào được đánh dấu là "Có tính phí" trong phần "Gói".
...
Bạn có thể [mua tín dụng](#buy-credits) trên trang Thanh toán của AI Studio. Trong quá trình mua, giao diện người dùng sẽ cung cấp số tiền tối thiểu cần mua trước theo yêu cầu của khu vực và cấp độ của bạn, cũng như số tiền tối đa mà bạn có thể có trong tài khoản tại một thời điểm.

### Tôi có thể thiết lập tài khoản trả trước để tự động mua thêm tín dụng khi cần không?

Có, bạn nên định cấu hình tính năng [tự động nạp tiền](#auto-reload) trong phần Cài đặt thanh toán của AI Studio. Bạn chỉ định số dư tín dụng "kích hoạt" (ví dụ: "khi số dư của tôi giảm xuống dưới 30 đô la") và "giá trị nạp tiền" (ví dụ: "thêm 100 đô la").

### Tôi có thể giới hạn số tiền được tự động nạp không?

Có. Người dùng trả trước có thể đặt [Hạn mức tính phí tự động hằng tháng](#monthly-auto-charge-limit) trong tiện ích **Tự động nạp tiền**. Khi tổng số tiền tự động nạp trong một chu kỳ thanh toán đạt đến hạn mức này, hệ thống sẽ tắt tính năng tự động nạp tiền cho đến tháng tiếp theo. Các giao dịch mua tín dụng theo cách thủ công không được tính vào hạn mức này.
...
Tất cả các dịch vụ Gemini API trong mọi dự án được thanh toán bằng tài khoản Thanh toán trước trên Google Cloud đó sẽ ngừng hoạt động ngay lập tức để tránh phát sinh thêm phí. Các dự án của bạn sẽ không tự động hạ cấp xuống Bậc miễn phí.
...
Để tránh bị tạm dừng dịch vụ Gemini API, bạn nên thiết lập tính năng [tự động nạp tiền](#auto-reload) để tự động mua thêm tín dụng khi số dư tín dụng của bạn giảm xuống dưới một giá trị mà bạn chỉ định.
...
Khi bạn nâng cấp lên phương thức [Trả sau](#postpay), Cloud Billing sẽ đóng tài khoản thanh toán Trả trước của bạn, tắt tính năng [tự động nạp tiền](#auto-reload) và tự động hoàn trả mọi khoản tín dụng Trả trước chưa sử dụng cho bạn (tuỳ thuộc vào thời gian xử lý hoàn tiền tiêu chuẩn).
...
Có, bạn có thể xem chi phí của Gemini API cùng với chi phí liên quan đến mọi dịch vụ khác của Google Cloud mà tài khoản thanh toán Cloud của bạn thanh toán trên [các trang Quản lý chi phí](https://docs.cloud.google.com/billing/docs/how-to/split-charging-cycle?hl=vi#cost-reports) trong [bảng điều khiển Thanh toán trên Cloud](https://console.cloud.google.com/billing?hl=vi). Xin lưu ý rằng bạn chỉ có thể quản lý số dư tín dụng trả trước trong AI Studio.
