...
**目标用户**：Wesley + 任何接手这个 KB 的 LLM（Claude Code / Codex / Cursor / 其他）。
...
│  │   ├── My-Setup/          Wesley 自己的 plugin/skill 配置笔记 │
...
类似 LLM-Wiki 的 Henry 1-1 prep / weekly report 模板。当前 03_Output/templates 是空的，等 cheatsheet 数量够多后再加自动模板。
...
| 拉远程更新（GHA cron 跑完后）| `cd ~/Desktop/wesleywufp/LLM-\ wiki-\ Claude/ && git pull` |
...
### #8 · 多 Mac iCloud workflow

此 repo 在 `~/Desktop/wesleywufp/`，git 自动走 wesleywufp-sketch 账号（SSH 路由配置见 `~/.gitconfig` includeIf）。**不要 hardcode `/Users/wesleywu/...`** —— 另一台 Mac 用户名可能不同。
